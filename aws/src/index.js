...
const main = async () => {
  ...
  try {
    ...
    await makePostgresService(ecs, FileSystemId, "postgres-task");
    await makeRabbitmqService(ecs, FileSystemId, "rabbitmq-task");
    await makeClickhouseService(ecs, FileSystemId, "clickhouse-task");
    await makeAgentApiService(ecs, FileSystemId, "agent-api-task");
    await makeSessionEnderService(
      ecs,
      FileSystemId,
      "session_ender-task"
    );
    await makeReplayerService(ecs, FileSystemId, "replayer-task");
